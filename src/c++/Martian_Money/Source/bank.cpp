...
unsigned int Bank::Trade(unsigned int Offer_Type, unsigned int Offer_Adjective,
	...
	if(Offer_Type<0 || Offer_Type>=M_Money.Get_Width() ||
			Offer_Adjective<0 ||
			Offer_Adjective>=M_Money.Get_Height() ||
			Receive_Type<0 || Receive_Type>=M_Money.Get_Width() ||
			Receive_Adjective<0 ||
...
double Bank::Get_Rate(unsigned int Type, unsigned int Adjective) const {
	if(Adjective>=M_Rate.Get_Height() || Adjective<0 ||
			Type>=M_Rate.Get_Width() || Type<0) {
